/**
 * Fetches footer links from the backend settings API, resolved per language.
...
export default async function () {
  ...
  try {
    ...
    console.warn("[openblog] Could not fetch languages for footer data");
  }
  ...
  for (const lang of languages) {
    try {
      const res = await fetch(
        `${BACKEND_URL}/api/v1/config/footer?lang=${lang.code}`,
      );
      if (res.ok) {
      ...
      console.warn(
        `[openblog] Could not fetch footer links for lang ${lang.code}`,
      );
      byLang[lang.code] = FALLBACK;
    }
  }
